...
# AunooAI Cloud Storage Volume Mount Script
# -----------------------------------------
# This script adds Cloud Storage volume mounts to existing AunooAI deployments on Cloud Run.
# It can be used to update deployments that were created without volume mounts.
...
for TENANT in $TENANTS; do
  ...
  if [ -z "$STORAGE_BUCKET" ]; then
    ...
    echo "Please run ./scripts/deploy-to-cloud-run.sh --project $PROJECT_ID --tenant $TENANT first"
    TENANTS_FAILED+=("$TENANT")
    continue
  fi
