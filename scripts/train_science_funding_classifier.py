#!/usr/bin/env python3
"""
Train a multi-label DeBERTa classifier for science funding categories.

Distills LLM classifications (gpt-4.1-mini) into a fast local DeBERTa model.
Follows the same pattern as train_policy_classifier.py.

Usage:
    python scripts/train_science_funding_classifier.py
    python scripts/train_science_funding_classifier.py --epochs 10 --batch-size 8
    python scripts/train_science_funding_classifier.py --model roberta-large --output models/science_funding_classifier/final_roberta_large
...
DEFAULT_OUTPUT_DIR = BASE_DIR / "models/science_funding_classifier"

# Default model - DeBERTa-base works reliably (no fp16 overflow issues)
DEFAULT_MODEL = "microsoft/deberta-base"

# Categories (loaded from export but defined here as fallback)
CATEGORIES = [
    "Grant Freezes & Cuts",
    "NIH & Biomedical",
    "NSF & Basic Science",
...
def load_training_data():
    ...
    train_path = DATA_DIR / "science_funding_train.json"
    val_path = DATA_DIR / "science_funding_val.json"
    test_path = DATA_DIR / "science_funding_test.json"
    mapping_path = DATA_DIR / "science_funding_label_mappings.json"

    for p in [train_path, val_path, test_path]:
        if not p.exists():
            print(f"Missing: {p}")
            print("Run: python scripts/export_science_funding_training_data.py")
    ...
    for i, cat in enumerate(categories):
        ...
        pct = count / total * 100
        # Sqrt-scaled inverse frequency, capped at 3.0 (same as policy classifier)
...
def train(
    ...
    train_ds, val_ds, test_ds = create_datasets(
        train_data, val_data, test_data, tokenizer, max_length
    )
    ...
    training_args = TrainingArguments(
        ...
        save_total_limit=2,
        fp16=False,  # DeBERTa has fp16 overflow issues
        bf16=False,
    ...
    # Save category mapping and metrics
    config = {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train science funding classifier")
