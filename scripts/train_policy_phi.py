#!/usr/bin/env python3
"""
Fine-tune Microsoft Phi-3-mini for policy classification using QLoRA.

Phi-3-mini is a 3.8B parameter model that's very efficient and fast.
Good for deployment where inference speed matters.

Usage:
    python scripts/train_policy_phi.py
...
OUTPUT_DIR = BASE_DIR / "models/policy_phi"

# Model choice - Phi-3-mini is efficient and fast (3.8B params)
# Options: "microsoft/Phi-3-mini-4k-instruct", "microsoft/Phi-3.5-mini-instruct"
...
# Training hyperparameters - optimized for Phi-3-mini (smaller model = larger batches)
...
LEARNING_RATE = 2e-4  # Phi responds well to slightly higher LR
NUM_EPOCHS = 5  # Good balance for smaller model
LORA_R = 16  # Phi-3 works well with smaller LoRA rank
LORA_ALPHA = 32
LORA_DROPOUT = 0.05
...
def train():
    ...
    lora_config = LoraConfig(
        ...
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_up_proj", "down_proj"],  # Phi-3 layer names
    )
