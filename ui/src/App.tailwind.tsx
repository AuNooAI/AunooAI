      ...
      <div className="space-y-3">
        {data.next_steps.map((step, idx) => {
          ...
          const stepText = typeof step === 'string'
            ? step
            : (step as any).action || (step as any).description || (step as any).priority || JSON.stringify(step);
          ...
          return (
            <div key={idx} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg hover:bg-pink-50 transition">
              ...
              <div className="flex-1">
                <p className="text-gray-800">{stepText}</p>
                {typeof step === 'object' && (step as any).timeline && (
                  <p className="text-sm text-gray-600 mt-1">Timeline: {(step as any).timeline}</p>
                )}
                {typeof step === 'object' && (step as any).stakeholders && (
                  <p className="text-sm text-gray-600 mt-1">
                    Stakeholders: {Array.isArray((step as any).stakeholders)
                      ? (step as any).stakeholders.join(', ')
                      : (step as any).stakeholders}
                  </p>
                )}
              </div>
            </div>
