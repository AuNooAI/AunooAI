...
export interface AuspexOpenEventDetail {
  ...
  topic?: string;  // Topic filter for Auspex context
}
 ...
 * @param topic Optional topic filter for Auspex context
 */
export function openAuspexWithQuery(query: string, autoSend: boolean = false, topic?: string): void {
  const event = new CustomEvent<AuspexOpenEventDetail>(AUSPEX_OPEN_EVENT, {
    detail: { query, autoSend, topic },
    bubbles: true,
    cancelable: true,
  });
