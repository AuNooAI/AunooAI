...
const SECTION_CONFIG: Record<string, { icon: typeof TrendingUp; color: string; bgColor: string; borderColor: string }> = {
  ...
  'Forward-Looking Insights': { icon: Lightbulb, color: 'text-pink-600 dark:text-pink-400', bgColor: 'bg-pink-50 dark:bg-pink-900/20', borderColor: 'border-pink-200 dark:border-pink-800' },
  'Forward-Looking Concerns': { icon: Lightbulb, color: 'text-pink-600 dark:text-pink-400', bgColor: 'bg-pink-50 dark:bg-pink-900/20', borderColor: 'border-pink-200 dark:border-pink-800' },
...
function parseNarrativeIntoSections(narrative: string): { title: string; content: string }[] {
  ...
  const isSectionHeader = (line: string): string | null => {
    ...
    if (trimmed.startsWith('# ') && !trimmed.startsWith('# Policy Tracker')) {
      return trimmed.replace(/^# /, '').replace(/\*\*/g, '').trim();
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (sectionTitle) {
      ...
      if (currentTitle) {
        ...
        if (preContent && preContent.length > 50 && !preContent.match(/^(Policy Tracker|Time Period|Analysis of|\*\*Time Period)/i)) {
          sections.push({
            title: 'Overview',
            content: preContent
          });
      ...
      if (!foundFirstSection && (
        trimmed.startsWith('Policy Tracker') ||
        trimmed.startsWith('Time Period:') ||
        trimmed.startsWith('**Time Period') ||
        trimmed.startsWith('**Policy Tracker') ||
        trimmed.match(/^#{1,2}\s*Policy Tracker/) ||
        trimmed === '' ||
        trimmed === '---'
      )) {
        continue;
  ...
  const filteredSections = sections.filter(section => {
    ...
    if (section.title === 'Overview') {
      ...
      if (content.includes('policy tracker insights') && section.content.length < 300) return false;
...
export function PolicyInsightsTab({ categories, topic, daysBack, refreshTrigger }: PolicyInsightsTabProps) {
  ...
  const handleDownloadNarrative = useCallback(() => {
    ...
    const filename = `policy-tracker-report-${dateStr}.md`;

    const content = `# Policy Tracker Narrative Analysis
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="relative overflow-hidden p-4 bg-gradient-to-br from-pink-500 to-pink-600 rounded-xl text-white">
                  ...
                  <div className="relative">
                    <span className="text-xs font-medium text-pink-100 uppercase tracking-wide">Articles</span>
                ...
                <div className="relative overflow-hidden p-4 bg-gradient-to-br from-violet-500 to-violet-600 rounded-xl text-white">
                  <Tag className="absolute -right-2 -bottom-2 w-16 h-16 text-violet-400/30" />
                  <div className="relative">
                    <span className="text-xs font-medium text-violet-100 uppercase tracking-wide">Categorized</span>
                ...
                <div className="relative overflow-hidden p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl text-white">
                  <Sparkles className="absolute -right-2 -bottom-2 w-16 h-16 text-emerald-400/30" />
                  <div className="relative">
              ...
              <AIDisclosureFooter
                ...
                purpose="To analyze policy tracker data through a critical lens, identifying patterns of authoritarian consolidation, democratic backsliding, and institutional threats"
              />
            </div>
          )}
        </div>
