/**
 * Geopolitical Schedule Modal - Configure Scheduled Hotspots Processing
 */
...
import {
  ...
  runScheduleNow,
  getAvailableTopics,
  type GeopoliticalSchedule,
  type AvailableTopic,
} from '../../services/geopoliticalHotspotsApi';

interface GeopoliticalScheduleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onScheduleRun?: () => void;
  model?: string;
...
export function GeopoliticalScheduleModal({
  isOpen,
  onClose,
  onScheduleRun,
  model = 'gpt-5.4-mini',
  ...
  const handleCreate = async () => {
    ...
    try {
      await createSchedule({
        ...
        process_all: formData.process_all,
  ...
  const handleToggleEnabled = async (schedule: GeopoliticalSchedule) => {
    try {
  ...
  const startEditing = (schedule: GeopoliticalSchedule) => {
    ...
    setFormData({
      ...
      schedule_unit: schedule.schedule_unit || 'hours',
        ...
        <div className="p-4 overflow-y-auto max-h-[70vh]">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="space-y-3 mb-4">
                {schedules.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <p className="text-sm">Create a schedule to automatically process articles.</p>
                  </div>
                ) : (
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            <button
                              ...
                              disabled={saving}
                              className="flex items-center gap-2 px-3 py-1.5 text-sm bg-pink-500 text-white rounded-lg hover:bg-pink-600 disabled:opacity-50"
                            >
                        ...
                        <div>
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <p className="text-sm text-gray-500 dark:text-gray-400">
                                {schedule.topic || 'Geopolitical Hotspots'} &bull; {schedule.batch_size} articles
                              </p>
                            </div>
                            <button
                          ...
                          {schedule.last_run_at && (
                            <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                              ...
                              Processed {schedule.last_run_articles_processed} articles,{' '}
                              {schedule.last_run_hotspots_created} created, {schedule.last_run_hotspots_updated} updated
                            </div>
                          )}
                          ...
                          <div className="flex justify-end gap-2 mt-3">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1.5 text-sm bg-pink-500 text-white rounded-lg hover:bg-pink-600 disabled:opacity-50"
                            >
                              {running === schedule.id ? (
              ...
              {showCreateForm ? (
                <div className="p-4 rounded-lg border border-pink-200 dark:border-pink-800 bg-pink-50/50 dark:bg-pink-900/10">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 p-3 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg text-gray-500 dark:text-gray-400 hover:border-pink-400 hover:text-pink-500 transition-colors"
                >
...
function ScheduleForm({
  ...
  formData: typeof import('./GeopoliticalScheduleModal').default extends React.FC<infer P> ? never : any;
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g., Daily Hotspots Update"
          className="w-full px-3 py-1.5 text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
        />
      </div>
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-3 py-1.5 text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
        >
          <option value="">Geopolitical Hotspots (default)</option>
          {availableTopics.map((t) => (
            <option key={t.topic} value={t.topic}>
              {t.topic} ({t.unprocessed_count} unprocessed)
            </option>
          ))}
        </select>
      </div>
        ...
        <div>
          ...
          >
            {BATCH_SIZES.map((size) => (
              <option key={size} value={size}>{size} articles</option>
            ))}
          </select>
        </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            checked={formData.process_all}
            onChange={(e) => setFormData({ ...formData, process_all: e.target.checked })}
            className="w-4 h-4 text-pink-500 border-gray-300 rounded focus:ring-pink-500"
          />
          <span className="text-sm text-gray-600 dark:text-gray-300">Reprocess all</span>
        </label>
      </div>
    </div>
  );
