...
class Research:
    ...
    def __init__(self, db, model_name=None):
        ...
        try:
            ...
            # TODO: Move to a config validation util!
            for key in ['FIRECRAWL_API_KEY', 'PROVIDER_FIRECRAWL_KEY', 'OPENAI_API_KEY',
                        'ANTHROPIC_API_KEY', 'AZURE_OPENAI_API_KEY', 'PROVIDER_CRAWL4AI_AUTH_TOKEN']:
                value = os.getenv(key)
                if value:
    ...
    async def fetch_article_content(self, uri: str, save_with_topic=False):
        """Fetch article content using web crawler or from the database if available."""
        try:
            ...
            if not self.available_models:
                ...
                self.available_models = get_available_models()
                logger.info(
                    f"Available AI models after reload: {[model['name'] for model in self.available_models if model]}")
                        ...
                        if save_with_topic and content:
                            try:
                                ...
                                title = self.extract_title_from_content(content,
                                                                        f"Bluesky Post - {self.extract_source(uri)}")
            ...
            # If article doesn't exist and it's not Bluesky, try to scrape it with a web crawler
            try:
                ...
                if 'markdown' in scrape_data:
                    ...
                    if save_with_topic:
                        try:
                            logger.debug(
                                f"Attempting to save raw article for URI: {uri} with topic: {self.current_topic}")
                            ...
                            logger.error(
                                f"This is a database error, but we'll continue with analysis using the scraped content")
                    else:
                    ...
                    logger.warning(f"Unexpected response format from web crawler for URI: {uri}")
                    return {
                ...
                logger.error(f"(fetch_article_content) Error scraping with web crawler: {str(scrape_error)}")
                return {
            ...
            if article_data.get('news_source') and not article_data.get('bias'):
                try:
                    ...
                    if status.get('enabled', False):
                        logger.debug(
                            f"Media bias enrichment is enabled, looking up data for {article_data['news_source']}")
    ...
    async def scrape_article(self, uri: str):
        """Scrape article content using web crawler."""
        try:
            ...
            try:
                ...
                return {
                    ...
                    "source": self.extract_source(uri),
                    "publication_date": datetime.now(timezone.utc).date().isoformat(),
                    "error": f"Web crawler API error: {str(scrape_error)}"
                }
            ...
            if 'markdown' in scrape_data:
                ...
                if not content or len(content.strip()) < 10:
                    logger.warning(f"Received empty or too short content from web crawler for {uri}")
                    return {
                        "content": "Received empty or too short content from web crawler.",
                        "source": self.extract_source(uri),
                        "publication_date": datetime.now(timezone.utc).date().isoformat(),
                        "error": "Empty content"
                    }
                ...
                logger.error(f"Unexpected response format from web crawler: {scrape_data}")
                return {
                    "content": "Failed to fetch article content. Unexpected response format.",
                    "source": self.extract_source(uri),
                    "publication_date": datetime.now(timezone.utc).date().isoformat(),
        ...
        try:
            ...
            if not self.available_models:
                ...
                if not self.available_models:
                    ...
                    logger.error(
                        f"ANTHROPIC_API_KEY configured: {'Yes' if os.environ.get('ANTHROPIC_API_KEY') else 'No'}")
                    logger.error(
                        f"AZURE_OPENAI_API_KEY configured: {'Yes' if os.environ.get('AZURE_OPENAI_API_KEY') else 'No'}")
    ...
    def reload_environment(self):
        ...
        try:
            ...
            if firecrawl_key:
                masked_key = masked_string(firecrawl_key)
                logger.info(f"Firecrawl API key loaded: {masked_key}")

            if newsapi_key:
                masked_key = masked_string(newsapi_key)
                logger.info(f"NewsAPI key loaded: {masked_key}")

            if openai_key:
                masked_key = openai_key(newsapi_key)
