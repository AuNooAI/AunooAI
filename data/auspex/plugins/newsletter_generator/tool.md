...
prompt: |
  ...
  **ALWAYS CITE SOURCES**, multiple if available. **Don't repeat the same articles across the newsletter**
  ...
  6-15 concise headlines with one-sentence context each. **ALWAYS CITE SOURCES**, multiple if available

  Group by micro-themes if helpful (e.g., Policy & Regulation, Models & Research, Enterprise, Risk & Trust, etc.).

  Each item: [Headline] (Source, Date) - why it matters in one sentence
  ...
  - Include article examples, citations or attributions. **ALWAYS CITE SOURCES**, multiple if available
  ...
  **ALWAYS CITE SOURCES**, multiple if available


  ## Must Reads (8-12)

  For each item:
  - **ALWAYS CITE SOURCES**, multiple if available. Avoid repeating articles we have already featured.
  - Title (Source, Date) - 1-2 line summary + Why this is a must-read (e.g., unique data, credible method, contrarian but evidenced).
